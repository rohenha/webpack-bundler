...
export default class Website extends mmodule {
  ...
  afterOnce() {
    const debounceResize = debounce(this.resize.bind(this, false), 600)
    ...
    const isAnimating = this.el.dataset.animate !== undefined
    if (isDebug || isAnimating) {
  ...
  after() {
    if (this.initalized) {
      this.call('updateGaPage', null, 'Axeptio')
    }
  ...
  enter({ current, next }) {
    ...
    this.el.classList.remove(`t-${current.namespace}`)
    this.el.classList.add(`t-${next.namespace}`)

    this.updateNav(next.url)
  ...
  updateNav(url) {
    ...
    for (let i = length; i >= 0; i -= 1) {
      ...
      element.classList[element.href === url.href ? 'add' : 'remove']('-active')
    }
  }

  updateExperience({ name, state }) {
    this.experience[name] = state
    this.el.classList[state ? 'add' : 'remove'](`-${name}`)
  }
}
